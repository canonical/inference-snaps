...
Inferencing with a CPU is always a good starting point and a simple way to do that is via a {spellexception}`llama.cpp`-based runtime. 
...
Most environment variables are best set within their scope. 
However, some variables are useful to set globally for the snap's runtime environment.

Add the following to set commonly needed variables globally:
```{literalinclude} create-inference-snap/snap/snapcraft.yaml
:caption: snap/snapcraft.yaml
:language: yaml
...
The CLI takes provides hardware detection, engine selection, component installation, server startup, config management, and more.
...
The above script loads a few configurations and starts the HTTP server.
...
We don't need all the artifacts, but that's okay for now. 
You can refine the organize step later to only include what is necessary.
...
We want the snap to run the server provided by a selected engine, as a background service.
...
The snap that you built will only work on the same architecture as the one you built it on.
