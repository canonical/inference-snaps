...
In this tutorial, you will create an inference snap for a large language model (LLM).
...
But first, you need to pick an LLM. Gemma 3 is a good choice for this tutorial because it is open and available in small sizes.
Gemma 3 is free and permits responsible commercial use.
Read its full terms of use [here](https://ai.google.dev/gemma/terms).

Inference with a CPU is always a good starting point, and a simple way to do it is with a {spellexception}`llama.cpp`-based runtime. 
...
You can use the [{spellexception}`llama.cpp` HTTP server](https://github.com/ggml-org/llama.cpp/tree/master/tools/server) implementation which supports model weights in the GGUF format.
...
Organize the files in the directory structure by their purpose:
...
Use the [kebab-case](https://developer.mozilla.org/en-US/docs/Glossary/Kebab_case) naming convention.
...
Runtimes and model weights are considered optional building blocks of this snap. 
This does not make perfect sense when there is only one for each.
But imagine this snap growing to have multiple of each, selected and downloaded on runtime based on the host machine.

In inference snaps, the optional building blocks are packages into [snap components](https://snapcraft.io/docs/components). 
This enables building the snap with all the building blocks but installing only what is relevant.
...
Now, the {file}`components` directory should look like this:
```{terminal}
...
If you build the snap now, {command}`snapcraft` will create the `.snap` and two `.comp` files. 
...
````{tip}
The {spellexception}`llama.cpp` engine you created also includes a chat web GUI.
